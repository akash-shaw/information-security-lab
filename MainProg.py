...
def system22_des_cbc():
    ...
    display_summary("DES-CBC", {'Original': data, 'IV': iv.hex(), 'Encrypted': encrypted.hex()[:60] + '...',
                                'Decrypted': decrypted})
...
def system23_3des_cbc():
    ...
    display_summary("3DES-CBC", {'Original': data, 'IV': iv.hex(), 'Encrypted': encrypted.hex()[:60] + '...',
                                 'Decrypted': decrypted})
...
def system25_aes_cbc():
    ...
    display_summary("AES-CBC", {'Original': data, 'IV': iv.hex(), 'Encrypted': encrypted.hex()[:60] + '...',
                                'Decrypted': decrypted})
...
def system26_aes_gcm():
    ...
    display_summary("AES-GCM", {'Original': data, 'Nonce': nonce.hex(), 'Tag': tag.hex(),
                                'Encrypted': encrypted.hex()[:60] + '...', 'Decrypted': decrypted})
...
def system27_rsa_encrypt():
    ...
    display_summary("RSA Encryption",
                    {'Original': data[:190], 'Encrypted': encrypted.hex()[:60] + '...', 'Decrypted': decrypted})
...
def system28_rsa_sign():
    ...
    display_summary("RSA Signature", {'Data': data, 'Signature': signature.hex()[:60] + '...',
...
class CustomCombinationBuilder:
        ...
        for ao in self.asym_operations:
            ...
            try:
                if asym_type == 'RSA_Encrypt':
                    ...
                    data_int = int.from_bytes(data_bytes[:4], 'big') if len(data_bytes) >= 4 else int.from_bytes(
                        data_bytes, 'big')
    ...
    def display_result(self, result):
        ...
        print(
            f"Total Algorithms Used: {len(result['hash_results']) + len(result['sym_results']) + len(result['asym_results'])}")
        print('=' * 80)
                ...
                if 'error' in ar:
                    ...
                    if 'signature' in ar:
                        if isinstance(ar['signature'], tuple):
                            print(
                                f"     Signature: (r={str(ar['signature'][0])[:30]}..., s={str(ar['signature'][1])[:30]}...)")
                        else:
